...
var languageList = [["auto", "auto"], ["zh-Hans", "zh-CN"], ["zh-Hant", "zh-TW"], ["en", "en"]];
new Map(languageList.map((_ref) => {
  var [standardLang, lang] = _ref;
  return [lang, standardLang];
}));
function getSupportLanguages() {
  return languageList.map((_ref2) => {
    var [standardLang] = _ref2;
    return standardLang;
  });
}
...
var historyFileName = "history.dat";
var chatGPTUserRole = "user";
var chatGPTAssistantRole = "model";
var newConversationTriggerWord = "SNC!!!";
var startNewConversation = "History conversation deleted! Start a new conversation now~~";
function _translate(_x) {
  return _translate2.apply(this, arguments);
}
function _translate2() {
  _translate2 = _asyncToGenerator(function* (text) {
    ...
    var cacheKey = "".concat(text).concat(from).concat(to);
    if (cache === "enable") {
      var _cacheData = resultCache.get(cacheKey);
      if (_cacheData)
        return _cacheData;
    } else {
      resultCache.clear();
...
function getFilePath(fileName) {
  return "$sandbox/".concat(fileName);
}
...
function translate(query, completion) {
  ...
  var str = formatString(text);
  var params = {
    from: detectFrom,
    to: detectTo,
    cache: api.getOption("cache")
  };
  var res = _translate(str, params);
  res.then((result) => completion({
    result
  })).catch((error) => {
    api.$log.error(JSON.stringify(error));
    if (error !== null && error !== void 0 && error.type)
      return completion({
        error
      });
    completion({
      error: util.error("api", "\u63D2\u4EF6\u51FA\u9519", error)
    });
  });
}
